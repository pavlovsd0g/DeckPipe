...
var tracks = [];
var pollTimer = null;
var jobsCollapsed = false;
var queueSnapshot = [];
var jobsGeneration = 0;
var cancellingJobs = /* @__PURE__ */ new Set();
var toastTimers = /* @__PURE__ */ new Map();
var logsTimer = null;
var logsGeneration = 0;
var logsEntries = [];
var logsBefore = null;
var logsHistory = false;
...
var BUTTON_HINTS = Object.freeze({
...
function showStatus(message) {
  showToast("statusRegion", message, 6e3);
}
...
function showError(error, fallbackKind) {
  ...
  showToast("errorRegion", describeError(error, fallbackKind), 12e3);
}
...
function recordUiEvent(event) {
  window.setTimeout(() => api("/api/logs/events", { body: { event } }).catch(() => {
  }), 0);
}
...
function replaceChildren(node, children) {
  node.replaceChildren(...(Array.isArray(children) ? children : [children]).filter((child) => child !== null && child !== void 0));
}
...
function formatRbSyncCounts(result) {
  ...
  if (result?.plan?.operation_kind === "sync") {
    return `добавить в плейлист: ${rbCountValue(counts, "add")}; уже есть: ${rbCountValue(counts, "already_present")}; использовать из коллекции: ${rbCountValue(counts, "reuse")}; импортировать: ${rbCountValue(counts, "import")}; сохранить дополнительных: ${rbCountValue(counts, "preserved")}. Существующие треки и порядок сохраняются`;
  }
...
function rbOrderedPathLines(result) {
  ...
  const rows = [...(additive ? result?.plan?.add : result?.plan?.desired_resolved) || []].sort((left, right) => Number(left.position || 0) - Number(right.position || 0));
  if (!rows.length) return [additive ? "Новых треков для добавления нет." : "Порядок файлов: плейлист пуст."];
  return [additive ? "Добавляемые треки и пути:" : "Порядок и пути файлов:", ...rows.map((row, index) => `${Number(row.position) || index + 1}. ${row.title || row.provider_id || "Трек"} — ${additive && row.existing_path || row.path || "путь не определён"}`)];
}
...
function showRbAppliedResult(result, action) {
  if (result?.applied && result?.reconciled) {
    ...
    showStatus(`${action}: успешно. Изменения применены и проверены.${backup}` + (stateError ? " Пути записаны, но текущее состояние файлов не удалось обновить; повторите проверку состояния." : "") + (postCommitError ? " Основная запись завершена, но дополнительная обработка после неё не завершилась." : ""));
...
Исходники сохранятся. База Rekordbox на этом шаге не изменяется.`,
    ...
    if (!rbSyncHasChanges(preview)) {
      showStatus("Пути Rekordbox уже соответствуют выбранному режиму. Изменений нет.");
      return;
    }
    const confirmed = await confirmRbAction(
      operation,
...
function renderJobs(jobs) {
  ...
  $("#queueSummary").textContent = active.length ? `В работе: ${active.length} · завершено: ${jobs.length - active.length}` : jobs.length ? `Завершено: ${jobs.length}` : "Очередь пуста";
  ...
  replaceChildren($("#jobs"), jobs.map((job) => {
    ...
    const label = job.state === "cancelled" ? "Остановлена" : stopping ? "Останавливаем…" : job.state === "queued" ? "В очереди" : finished ? job.failed ? "Завершена с ошибками" : "Завершена" : "Загрузка";
    const cancel = !finished && ["append", "playlist_order"].includes(job.mode) ? button(stopping ? "Останавливаем…" : "Остановить", "cancel-job", { className: "ghost button-small", dataset: { id: job.id } }) : null;
...
function renderLogs() {
  ...
  const rows = logsEntries.map((entry) => {
    ...
    const context = ["job_id", "track_id", "playlist_id", "error_code"].filter((key) => entry[key] != null).map((key) => `${key}: ${entry[key]}`).join(" · ");
...
var clickActions = Object.freeze({
  ...
  "older-logs": () => {
    if (!logsLoading && logsBefore) return loadLogs({ append: true });
  },
