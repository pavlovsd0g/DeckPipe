...
Maintain an atomically written durable operation journal with operation identity, expected inputs, backup inventory/hashes and phases. Persist recovery data before the first mutation. Following interruption, ordinary sync/flip stops with a read-only recovery-needed result. A separate recovery preview identifies the exact interrupted operation; only confirmation of its current journal/database/backup/file hash may restore the recorded pre-operation rows under a held SQLite write transaction, then verify them through a fresh connection. A normal sync confirmation never authorizes recovery. The recovery entry is available without a selected playlist or a readable streaming/local source; fresh synchronization still requires a complete source. After recovery, require a fresh normal preview and confirmation. Keep database and affected-file ownership throughout restoration; do not replace master.db or remove WAL/SHM after releasing the lock. Validate that the schema supports this recovery before the original operation begins, refusing unsupported layouts. Detect damaged/missing backup, running Rekordbox, an unrecognized database or external changes and return a specific recovery-needed error; do not guess or discard the journal. Preview/status can report recovery needed but do not silently mutate the target. Exercise real subprocess termination between database and external-file boundaries.
...
Recovery uses `GET /api/rb/recovery` for read-only context and `POST /api/rb/recovery` with `expected_plan_hash` plus `RESTORE_REKORDBOX_OPERATION` for the separate confirmation. These routes accept no source, playlist or arbitrary database path. The journal binding is checked again while holding recovery ownership. Unknown Windows process status fails closed with a structured error; localized process output must not be decoded using an assumed UTF-8 locale.
