...
let rbSelectionEpoch = 0;
let rbOperationGeneration = 0;
let rbOperationBusy = false;
let rbMediaMode = null;
let activeDialogKind = null;
let pendingRbDialog = null;
const rbTargetIdsBySource = new Map();
...
async function api(path, opts = {}) {
  ...
  if (!response.ok) {
    ...
    let detail = typeof rawDetail === 'object' && rawDetail !== null
      ? (rawDetail.message || rawDetail.code || `HTTP ${response.status}`)
      : (rawDetail || response.statusText || `HTTP ${response.status}`);
...
function rbOperationIsCurrent(operation) {
  ...
  return !!operation && operation.generation === rbOperationGeneration && !!selected &&
    ...
    selected.title === operation.selection.title;
}
...
function configureRbDialog({title, children, confirmLabel, cancelLabel, value}) {
  ...
  setResultState(null);
  const promise = new Promise(resolve => { pendingRbDialog = {resolve, value}; });
  openDialog(accept);
  return promise;
}
...
function chooseRbTargetDialog(operation, title, candidates) {
  if (!rbOperationIsCurrent(operation)) return Promise.resolve(null);
  const select = create('select', {id: 'rbTargetSelect', className: 'rb-target-select',
    attrs: {'aria-label': `Целевой плейлист Rekordbox для ${title}`}},
  candidates.map(item => create('option', {value: String(item.id),
...
async function scanLibrary() {
  if (!libraryConfigured) { renderRootSetup(); return false; }
  const generation = ++libraryPollGeneration;
  libraryReady = false;
  let state = await api('/api/library/scan', {body: {}});
  while (generation === libraryPollGeneration) {
    $('#libraryState').textContent = state.state === 'scanning'
      ? `Сканируем общую библиотеку… файлов: ${state.files || 0}`
      : `В каталоге: ${state.files || 0} файлов${state.error ? ` · ${state.error}` : ''}`;
...
async function retryRemote(id) {
  ...
  } finally { if (tab === 'errors') await loadErrors(); }
}
...
function switchTab(nextTab) {
  ...
  setEmpty(nextTab === 'errors'
    ...
    : `Выберите ${nextTab === 'sc' ? 'источник' : nextTab === 'local' ? 'локальный плейлист' : 'плейлист'} слева`);
  loadPlaylists();
}
...
function renderSearch() {
  ...
  for (const [service, label] of [['deezer', 'Deezer'], ['sc', 'SoundCloud']]) {
    ...
    if (serviceData.errors?.length) sections.push(create('div', {className: 'live-region error-region',
...
async function loadScSources(isCurrent = beginPlaylistLoad()) {
  try { await api('/api/sc/sync-account', {body: {}}); } catch (error) {
    if (!isCurrent()) return;
...
async function loadTracks(playlist) {
  ...
  try {
    const url = playlist.kind === 'sc'
      ...
      : `/api/playlists/${canonicalSourceKey(playlist.kind, playlist.id)}/tracks?title=${encodeURIComponent(playlist.title)}`;
...
function renderTracks() {
  ...
  tracks.forEach((track, index) => {
    ...
    if (track.file_path) {
      ...
      locationCell.append(create('select', {id: `location-${index}`, attrs: {'aria-label': `Выберите файл для ${track.title}`}},
...
function selectedTracks(onlyMissing = false) {
  ...
  return selected.map(track => ({id: track.id, title: track.title, artist: track.artist, duration: track.duration, url: track.url,
    provider: track.provider || (current?.kind === 'sc' ? 'sc' : 'deezer'), total: tracks.length}));
}
...
async function syncPlaylistOrder() {
  ...
  if (!missing.length) {
    ...
    showStatus(`Порядок применён к файлам в папке плейлиста (переименовано: ${renumber.renamed}). ` +
      (unresolved ? `Требуют проверки: ${unresolved}.` : 'Все треки найдены в библиотеке.'));
    await rescan();
    return;
  }
...
function formatRbPreview(result, action = 'Синхронизация') {
  ...
  const targetText = !plan.target
    ? 'Цель Rekordbox не удалось определить.'
    : target.id == null
    ? `Цель Rekordbox: новый плейлист «${target.name || 'без названия'}» будет создан.`
...
async function flipWav() {
  ...
  try {
    ...
    if (toWav) {
      const prepareConfirmed = await confirmRbAction(operation, 'Подготовка WAV',
        `Подготовить отдельные проверенные WAV-файлы для «${selection.title}»?\nИсходники сохранятся. База Rekordbox на этом шаге не изменяется.`, 'Подготовить WAV');
      if (!rbOperationIsCurrent(operation)) return;
      if (!prepareConfirmed) {
