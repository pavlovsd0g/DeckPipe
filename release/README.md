...
- `release/prepare-wheelhouse.ps1` creates and uses only the fixed D-drive lab layout: `downloads`, `wheelhouse`, `tool-cache`, `build`, `staging`, `install`, `backups\rekordbox`, `qa-evidence`, and `logs-redacted`. It records a redacted host/tool inventory under `qa-evidence`, compiles both locks with hashes, downloads only binary wheels, writes `wheelhouse\wheelhouse-manifest.json`, and proves a clean offline install from that wheelhouse.
- `release/build.ps1` refuses to stage artifacts unless both Python lock files contain `--require-hashes` and real hashes, and unless the explicit wheelhouse manifest exactly matches the wheel files by relative name, size, and lowercase SHA-256. The wheelhouse must live below `D:\DeckPipe-RC-Lab`, outside the repository, and outside staging.
